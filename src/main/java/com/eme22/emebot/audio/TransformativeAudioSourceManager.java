...
public class TransformativeAudioSourceManager extends YoutubeAudioSourceManager
{
    ...
    public static List<TransformativeAudioSourceManager> createTransforms(Map<String, String> transforms)
    {
        try
        {
            return transforms.keySet().stream()
                    .map(configValue -> new TransformativeAudioSourceManager(configValue, ConfigFactory.parseString(configValue)))
                    .collect(Collectors.toList());
        }
        catch (Exception ex)
        {
